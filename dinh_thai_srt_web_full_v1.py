...
def is_translated_or_skip(text: str) -> bool:
...
def prepare_items(items: List[SubtitleItem]) -> Tuple[List[SubtitleItem], int]:
    skipped = 0
    for item in items:
        if is_translated_or_skip(item.text):
...
# GIAO DIỆN
# =========================
st.markdown('<div class="app-title">🎬 Đình Thái - Dịch SRT Pro</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="app-subtitle">Layout đẹp • Nhiều API key • Retry key lỗi • Preview câu dịch • Progress realtime • Download SRT</div>',
...
with left:
    ...
    st.markdown('<div class="note">Dán nhiều API key xuống dòng để tool tự xoay vòng khi gặp key lỗi.</div>', unsafe_allow_html=True)
                ...
                if total_batches == 0:
                    ...
                    with ThreadPoolExecutor(max_workers=max(1, len(worker_slots))) as executor:
                        ...
                        for future in as_completed(futures):
                            ...
                            preview_text = "\n\n".join(
                                f"Gốc: {item.text}\nDịch: {item.translated_text}" for item in batch[:3]
                            )
...
if st.session_state["finished"]:
    if st.session_state["had_error"]:
        st.markdown(
            '<div class="warn-box">Đã dịch xong phần thành công. Một số batch bị lỗi. Bạn vẫn có thể tải file hiện tại. Muốn xử lý phần lỗi chuẩn hơn, hãy chạy lại với file kết quả hiện tại hoặc dùng thêm API key ổn định hơn.</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
