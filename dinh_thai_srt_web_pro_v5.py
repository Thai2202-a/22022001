...
def build_prompt(batch: List[SubtitleItem], style_prompt: str, source_language: str, target_language: str) -> str:
    ...
    # THÊM: Logic xác định ngôn ngữ đích cụ thể
    final_target = target_language
    if target_language == "Giống ngôn ngữ nguồn (Sửa lỗi)":
        final_target = "ngôn ngữ gốc (chỉ sửa lỗi và tối ưu văn phong)"
...
with left:
    ...
    # CẬP NHẬT: Chia làm 4 cột để thêm Ngôn ngữ đích
    ...
    with c4:
        # THÊM: Dropdown chọn ngôn ngữ đích
    ...
    if not uploaded_files:
        ...
        if not api_keys:
            ...
            for f in uploaded_files:
                ...
                # TRUYỀN THÊM target_language vào tham số
...
if st.session_state["finished"]:
    if st.session_state["had_error"]:
        ...
        st.markdown(
            f'<div class="status-ok">Đã xử lý xong toàn bộ sang {target_language}. Bạn có thể tải file kết quả bên dưới.</div>',
            unsafe_allow_html=True
        )
...
if st.session_state["result_ready"] and st.session_state["zip_bytes"]:
    ...
    if st.session_state["single_bytes"]:
        st.download_button(
            ...
            file_name="output_translated.srt",
    ...
    st.markdown(
        ...
        <div class="done-popup">
            <div class="done-popup-body">
                ...
                <div class="done-text">
                    Toàn bộ quá trình dịch sang <b>{target_language}</b> đã hoàn thành.<br>
